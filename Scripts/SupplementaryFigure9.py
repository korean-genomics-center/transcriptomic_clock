...
numerical_columns.remove("r-GTP")
numerical_columns.remove("Anti-thrombin\xa0III")
...
for ax, column, corr, padj in zip(axes, numerical_columns, list_corrs, list_padjs):
    ...
    ax.text(0.05, 0.95, f"r = {round(corr, 3)}\nFDR = {padj:.1e}", transform=ax.transAxes,
...
plt.tight_layout()
plt.savefig(f"{WORKDIR}/Figures/Supplementary_Figure_9.tiff", dpi=300, bbox_inches="tight")
plt.savefig(f"{WORKDIR}/Figures/Supplementary_Figure_9.png", dpi=300, bbox_inches="tight")
plt.show()
plt.close()
# %%
