...
# %%
path_reg = "//BiO/Research/GeroExpressome/Workspace/RNAClock/Revision_final_trained_with_only_korean/LASSO_INFO/healthy_cohort_train_prev_median_filter/corr_0.35_above_pval_0.05_below/regression_results.txt"
...
# %%
path_norm_exp = "/BiO/Research/GeroExpressome/Workspace/RNAClock/Revision_final_trained_with_only_korean/FeatureTable/healthy_cohort_train_prev_median_filter.txt"
...
path_corr_table = "/BiO/Research/GeroExpressome/Workspace/RNAClock/Revision_final_trained_with_only_korean/FeatureTable/healthy_cohort_train_prev_median_filter_corr_table.txt"
if not os.path.exists(path_corr_table):
    df_norm_exp = pd.read_csv(path_norm_exp, sep="\t")
    df_norm_exp_set_idx = df_norm_exp.set_index("Project-ID")
    corr_matrix = df_norm_exp_set_idx.corr(method="pearson")
    corr_matrix.to_csv(path_corr_table, sep=",", index=True)
else:
    corr_matrix = pd.read_csv(path_corr_table, sep=",")
...
cg = sns.clustermap(
    corr_matrix_selected.T.loc[:, ~corr_matrix_selected.T.isna().any()],
    cmap="coolwarm",
    xticklabels=False,
    figsize=(5, 15),
    vmin=-1,
...
cg.ax_heatmap.set_xlabel("all genes", fontsize=12)
...
for feature in list_features:
    corr_feature = corr_matrix_selected[feature]
    corr_feature_abs = corr_feature.apply(abs)
    corr_feature_filt = corr_feature_abs[corr_feature_abs > 0.68]
