...
def draw_histogram_stat(df_qc, stats, outfile):
...
mode = "fastp"
project = "healthy"
dir_qc = "/BiO/Research/RNASeqReference/Workspace/KU10K/Resources/Data/QC/KU10K/StatsSummary"
dict_fastp_qc = {
...
conversion_table = "/BiO/Research/RNASeqReference/Workspace/KU10K/Results/AgingTranscriptomePaper1/QC/conversion_table.txt"
path_exp = f"/BiO/Research/RNASeqReference/Workspace/KU10K/Resources/Data/Expression/20231206/expression_matrix_genes.results_TPM.tsv"
...
df_stat_merge_reordered_filtered.to_csv(f"/BiO/Research/RNASeqReference/Workspace/KU10K/Results/AgingTranscriptomePaper1/QC/{mode}_qc_{project}.tsv", sep="\t", index=False)
...
conversion_table = "/BiO/Research/RNASeqReference/Workspace/KU10K/Results/AgingTranscriptomePaper1/QC/conversion_table.txt"
path_exp = f"/BiO/Research/RNASeqReference/Workspace/KU10K/Resources/Data/Expression/20231211/raw/{project}.txt"
list_samples_excl = ["KU10K-00922-A1", "KU10K-01025-A1", "KU10K-01027-A1", "KU10K-02029-A1"]
...
indir = "/BiO/Research/RNASeqReference/Workspace/KU10K/Results/AgingTranscriptomePaper1/QC/*_qc_ku10k.tsv"
outfile = "/BiO/Research/RNASeqReference/Workspace/KU10K/Results/AgingTranscriptomePaper1/QC/merged_qc_ku10k.tsv"

list_df_qc = list()
list_files_qc = glob.glob(indir)
for fileqc in list_files_qc[::-1]:
    df_qc = pd.read_csv(fileqc, sep="\t")
    list_df_qc.append(df_qc)

df_merged_qc = reduce(lambda x, y: x.merge(y, on="SampleID", how="outer"), list_df_qc)
df_merged_qc_sorted_id = df_merged_qc.sort_values(by=["SampleID"])
df_merged_qc_sorted_id["SubjectID"] = df_merged_qc_sorted_id["SampleID"].apply(lambda x: x.split("_")[0])
list_colheader = ["SubjectID"] + list(df_merged_qc.columns)
df_merged_qc_sorted_id_reordered = df_merged_qc_sorted_id[list_colheader]
df_merged_qc_sorted_id_reordered.to_csv(outfile, sep="\t", index=False)

# %% [fastp qc - COVID19]
mode = "fastp"
dir_qc = "/BiO/Research/RNASeqReference/Workspace/KU10K/Resources/Data/QC/COVID19"
dict_fastp_qc = {
    "summary___before_filtering___total_bases": "Total_Number_of_Bases_Before_Filtering",
    "summary___after_filtering___total_bases": "Total_Number_of_Bases_After_Filtering",
    "summary___before_filtering___read1_mean_length": "Read1_Mean_Length_Before_Filtering",
...
df_fastp_qc_sorted.to_csv(f"/BiO/Research/RNASeqReference/Workspace/KU10K/Results/AgingTranscriptomePaper1/QC/{mode}_qc_covid19.tsv", sep="\t", index=False)
...
df_star_qc_sorted.to_csv(f"/BiO/Research/RNASeqReference/Workspace/KU10K/Results/AgingTranscriptomePaper1/QC/{mode}_qc_covid19.tsv", sep="\t", index=False)
...
indir = "/BiO/Research/RNASeqReference/Workspace/KU10K/Results/AgingTranscriptomePaper1/QC/*_qc_covid19.tsv"
outfile = "/BiO/Research/RNASeqReference/Workspace/KU10K/Results/AgingTranscriptomePaper1/QC/merged_qc_covid.tsv"

list_df_qc = list()
list_files_qc = glob.glob(indir)
for fileqc in list_files_qc:
    df_qc = pd.read_csv(fileqc, sep="\t")
    list_df_qc.append(df_qc)
