...
#     path_exp_mtx = f"{WORKDIR}/Expression/{group}.txt"
#     path_metadata = f"{WORKDIR}/Metadata/{group}.txt"
#     dir_exp_out = f"{WORKDIR}/Expression/selected"
#     dir_meta_out = f"{WORKDIR}/Metadata/selected"
...
for group in list_groups:
    ...
    list_sample_id = get_list_sample_id(path_exp_mtx)
    dict_gene_exp = get_dictionary_gene_expression(path_exp_mtx)
    if flag_filter:
    ...
    if flag_find_median_exp:
        ...
        df_meta = read_file_metadata(path_metadata) 
